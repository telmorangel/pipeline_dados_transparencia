...
6. **Qual UF de destino aparece em mais trechos?**
   * *Abordagem Analítica:* Agregação geográfica por `destino_uf` em `silver_trecho`, identificando os polos estatuais que concentram o maior fluxo de deslocamento de servidores públicos.
...
```bash
# Fase 0: Geração do Schema e aplicação das tabelas DDL no banco:
python -c "import banco; banco.executar_sql_arquivo('0_criar_banco.sql')"
